/**
* @file Wifi_RL_env.cc
* @brief Enhanced realistic ns-3 simulation of a large-scale campus Wi-Fi network
* with a full Reinforcement Learning interface for agent training.
*/
...
private:
    ...
    double m_throughput_weight;
    double m_energy_penalty;
    double m_loss_rate_penalty;
    double m_qos_satisfaction_weight;

    // Reference to your environment
    OptimizedWiFiEnvironment* env;

public:
    RLInterface(int port_num, OptimizedWiFiEnvironment* environment) 
        : server_socket(-1), client_socket(-1), port(port_num), rl_enabled(false), 
          running(false), env(environment) {}
    
    void StartRLInterface() {
        if (!rl_enabled) return;
        
        server_socket = socket(AF_INET, SOCK_STREAM, 0);
        if (server_socket < 0) {
            NS_LOG_ERROR("Error creating socket");
            return;
        }
        ...
        NS_LOG_INFO("RL Interface listening on port " << port);
        
        running = true;
        communication_thread = std::thread(&RLInterface::HandleCommunication, this);
        // Start connection health checking
        Simulator::Schedule(Seconds(10), &RLInterface::CheckConnectionHealth, this);
    }

    double CalculateReward(const Json::Value& global_metrics);
    void ProcessStep();
    void HandleReset(const Json::Value& request);
    void HandleAction(const Json::Value& request);
    
    Json::Value GetAPStates();
    Json::Value GetGlobalMetrics();
    Json::Value GetInterferenceMatrix();
    void ApplyChannelAssignment(const Json::Value& channels);
    void ApplyPowerLevels(const Json::Value& power_levels);
    
    // Helper methods
    double GetAPUtilization(uint32_t ap_id);
    uint32_t GetNumUsers(uint32_t ap_id);
    double GetAverageDelay(uint32_t ap_id);
    double GetPowerConsumption(uint32_t ap_id);
    double GetInterferenceLevel(uint32_t ap_id);
    double GetThroughputFromFlowMonitor();
    ...
    double CalculateAverageQoS();
    void HandleCommunication();
    double CalculateInterference(uint32_t ap1, uint32_t ap2);
    bool CheckIfEpisodeDone();
    void SetAPPowerLevel(uint32_t ap_id, double power_level);
    
    void SetRewardWeights(double throughput, double energy, double loss, double qos) {
        m_throughput_weight = throughput;
        m_energy_penalty = energy;
        m_loss_rate_penalty = loss;
        m_qos_satisfaction_weight = qos;
    }

    void EnableRLInterface(bool enable) {
        rl_enabled = enable;
    }
...
struct DailyUsagePattern {
    ...
    DailyUsagePattern() {
        hourlyMultiplier = {
            0.1, 0.05, 0.05, 0.05, 0.1, 0.2,
            0.4, 0.8, 1.2, 1.5, 1.8, 1.9,
            2.0, 1.8, 2.2, 2.5, 2.2, 1.8,
            1.5, 1.2, 0.8, 0.6, 0.4, 0.2
        };
...
// CORRECTLY PLACED ENUM DEFINITION
enum DeviceType { DEVICE_LAPTOP = 0, DEVICE_SMARTPHONE = 1, DEVICE_TABLET = 2, DEVICE_IOT = 3 };
...
struct DeviceProfile {
    ...
    DeviceProfile(DeviceType t) : type(t) {
        switch(t) {
            case DEVICE_LAPTOP:
                wifiStandard = "802.11ax"; txPower = 20.0; powerManagementFactor = 0.8;
                antennaGain = 2.0; supportedApps = {"bulk_transfer", "video_streaming", "web_browsing"};
                break;
            case DEVICE_SMARTPHONE:
                wifiStandard = "802.11ac"; txPower = 15.0; powerManagementFactor = 0.9;
                antennaGain = 1.0; supportedApps = {"video_streaming", "social_media", "web_browsing"};
                break;
            case DEVICE_TABLET:
                wifiStandard = "802.11ac"; txPower = 18.0; powerManagementFactor = 0.85;
                antennaGain = 1.5; supportedApps = {"video_streaming", "web_browsing"};
                break;
            case DEVICE_IOT:
                wifiStandard = "802.11ac"; txPower = 10.0; powerManagementFactor = 0.95;
                antennaGain = 0.5; supportedApps = {"sensor_data"};
                break;
        }
    }
};
...
struct WeatherCondition {
    ...
    double fadeMargin;
    WeatherCondition(std::string n, double atten, double fade) : name(n), attenuationFactor(atten), fadeMargin(fade) {}
};
...
public:
    ...
    std::map<uint32_t, double> channelUtilization;
    std::vector<bool> staActiveStatus;
    std::map<uint32_t, double> apQueueDelay;
    std::map<uint32_t, double> channelInterference;

    const std::vector<uint32_t>& GetAvailableChannels() const {
        return availableChannels;
    }

    // --- RL Episode Management ---
    const double episodeDuration = 120.0; // Defines episode length in sim seconds
    Time m_episodeStartTime;
    uint32_t m_currentEpisodeStartHour;

private:
    const uint32_t nServers = 4;
    const double simTime = 240; // Total simulation runtime
    ...
    mutable std::map<uint32_t, std::vector<uint32_t>> channelToAPs;
...
void RLInterface::HandleReset(const Json::Value& request) {
    NS_LOG_INFO("Handling RL reset request");
    env->ResetEpisode();
    Simulator::Schedule(Seconds(0.01), &RLInterface::ProcessStep, this);
}

bool RLInterface::CheckIfEpisodeDone() {
    return (Simulator::Now() - env->m_episodeStartTime).GetSeconds() >= env->episodeDuration;
}

double RLInterface::CalculateReward(const Json::Value& global_metrics) {
    double throughput = global_metrics["total_throughput"].asDouble();
    double energy = global_metrics["total_energy_consumption"].asDouble();
    double loss_rate = global_metrics["packet_loss_rate"].asDouble();
    double qos = global_metrics["average_qos_satisfaction"].asDouble();

    double reward_component = (throughput * m_throughput_weight) + (qos * m_qos_satisfaction_weight);
    double penalty_component = (energy * m_energy_penalty) + (loss_rate * m_loss_rate_penalty);

    NS_LOG_INFO("Reward Calc: Throughput=" << throughput << ", Energy=" << energy << ", Loss=" << loss_rate);
    NS_LOG_INFO("Reward=" << reward_component << ", Penalty=" << penalty_component << ", Final=" << reward_component - penalty_component);

    return reward_component - penalty_component;
}

void RLInterface::ProcessStep() {
    EnsureConnection();
    if (client_socket < 0) {
        NS_LOG_WARN("Cannot send state: No connection to RL agent");
        return;
    }
    Json::Value response;
    response["type"] = "state";
    
    response["ap_states"] = GetAPStates();
    Json::Value global_metrics = GetGlobalMetrics();
    response["global_metrics"] = global_metrics;
    response["interference_matrix"] = GetInterferenceMatrix();
    
    response["reward"] = CalculateReward(global_metrics);
    response["done"] = CheckIfEpisodeDone();
    response["simulation_time"] = Simulator::Now().GetSeconds();
...
void RLInterface::HandleCommunication() {
    struct sockaddr_in client_addr;
    socklen_t client_len = sizeof(client_addr);
    
    // Set timeout for accept to prevent blocking indefinitely
    struct timeval tv;
    tv.tv_sec = 30;  // 30 second timeout for accept
    tv.tv_usec = 0;
    ...
    while (running) {
        ...
        if (client_socket < 0) {
            if (errno == EAGAIN || errno == EWOULDBLOCK) {
                ...
                NS_LOG_DEBUG("Accept timeout, checking if still running...");
                continue;
            } else {
                NS_LOG_ERROR("Accept failed: " << strerror(errno));
                break;
            }
        }
        
        NS_LOG_INFO("Python RL agent connected");
        
        // Set timeouts for the client socket
        tv.tv_sec = 10;  // 10 second timeout for recv/send
        ...
        while (running) {
            ...
            if (bytes_received <= 0) {
                if (bytes_received == 0) {
                    NS_LOG_INFO("Python client disconnected.");
                } else {
                    if (errno == EAGAIN || errno == EWOULDBLOCK) {
                        // Timeout, check if we should continue
                        continue;
                    } else {
                        NS_LOG_ERROR("recv error: " << strerror(errno));
                    }
                }
                break;
            }
            ...
            if (!reader.parse(message, request)) {
                NS_LOG_ERROR("Error parsing JSON message: " << message);
                ...
                Json::StreamWriterBuilder builder;
                std::string error_str = Json::writeString(builder, errorResponse);
                send(client_socket, error_str.c_str(), error_str.length(), 0);
                
                continue;
            }
            
            if (request["type"].asString() == "reset") {
                HandleReset(request);
            } else if (request["type"].asString() == "action") {
                HandleAction(request);
            } else if (request["type"].asString() == "terminate") {
                running = false;
                break;
            } else if (request["type"].asString() == "ping") {
                // Respond to ping requests for connection health checking
                Json::Value pongResponse;
                pongResponse["type"] = "pong";
                pongResponse["timestamp"] = Simulator::Now().GetSeconds();
                
                Json::StreamWriterBuilder builder;
                std::string pong_str = Json::writeString(builder, pongResponse);
                send(client_socket, pong_str.c_str(), pong_str.length(), 0);
            }
        }
...
void RLInterface::CheckConnectionHealth() {
    if (!running || client_socket < 0) {
        return;
    }
    ...
    Json::Value pingMsg;
    pingMsg["type"] = "ping";
    pingMsg["timestamp"] = Simulator::Now().GetSeconds();
    
    Json::StreamWriterBuilder builder;
    std::string ping_str = Json::writeString(builder, pingMsg);
    
    ssize_t bytes_sent = send(client_socket, ping_str.c_str(), ping_str.length(), 0);
    if (bytes_sent < 0) {
        NS_LOG_WARN("Connection health check failed: " << strerror(errno));
        close(client_socket);
        client_socket = -1;
        EnsureConnection();
    }
...
void RLInterface::HandleAction(const Json::Value& request) {
    // Validate action structure
    if (!request.isMember("channels") || !request["channels"].isArray() ||
        !request.isMember("power_levels") || !request["power_levels"].isArray()) {
        NS_LOG_ERROR("Invalid action format from RL agent");
        
        // Send error response
        Json::Value errorResponse;
        errorResponse["type"] = "error";
        errorResponse["message"] = "Invalid action format";
    ...
    Json::Value channels = request["channels"];
    Json::Value power_levels = request["power_levels"];
    
    // Validate channel values
    const std::vector<uint32_t>& availableChannels = env->GetAvailableChannels();
    for (const auto& channel : channels) {
        if (!channel.isUInt()) {
            NS_LOG_ERROR("Invalid channel value type");
            return;
        }
        uint32_t ch = channel.asUInt();
        if (std::find(availableChannels.begin(), 
                     availableChannels.end(), ch) == availableChannels.end()) {
            NS_LOG_ERROR("Invalid channel number: " << ch);
            return;
        }
    }
    
    // Validate power levels
    for (const auto& power : power_levels) {
        if (!power.isDouble() && !power.isInt()) {
            NS_LOG_ERROR("Invalid power value type");
            return;
        }
        double pwr = power.asDouble();
        if (pwr < 0 || pwr > 30) {  // Reasonable power range
            NS_LOG_ERROR("Power level out of range: " << pwr);
            return;
        }
    }
    
    NS_LOG_INFO("Received valid action from RL agent. Applying to network...");
    
    ApplyChannelAssignment(channels);
    ApplyPowerLevels(power_levels);
    
    const double simulation_step_time = 1.0;
    Simulator::Schedule(Seconds(simulation_step_time), &RLInterface::ProcessStep, this);
}

Json::Value RLInterface::GetAPStates() {
    Json::Value ap_states(Json::arrayValue);
    for (uint32_t i = 0; i < env->nAPs; ++i) {
        Json::Value ap_state;
        ap_state["utilization"] = GetAPUtilization(i);
        ap_state["num_users"] = GetNumUsers(i);
        ap_state["avg_delay"] = GetAverageDelay(i);
        ap_state["power_consumption"] = GetPowerConsumption(i);
        ap_state["interference_level"] = GetInterferenceLevel(i);
        ap_states.append(ap_state);
    }
    return ap_states;
}

Json::Value RLInterface::GetGlobalMetrics() {
    Json::Value metrics;
    metrics["total_throughput"] = CalculateTotalThroughput();
    metrics["packet_loss_rate"] = CalculatePacketLossRate();
    metrics["total_energy_consumption"] = CalculateTotalEnergyConsumption();
    metrics["energy_efficiency"] = CalculateEnergyEfficiency();
    metrics["average_qos_satisfaction"] = CalculateAverageQoS();
    return metrics;
}

Json::Value RLInterface::GetInterferenceMatrix() {
    Json::Value matrix(Json::arrayValue);
    for (uint32_t i = 0; i < env->nAPs; ++i) {
        Json::Value row(Json::arrayValue);
        for (uint32_t j = 0; j < env->nAPs; ++j) {
            double interference = CalculateInterference(i, j);
...
void RLInterface::EnsureConnection() {
    if (client_socket >= 0 || !running) {
        return; // Already connected or not running
    }

    NS_LOG_INFO("Attempting to reconnect to RL agent...");
    ...
    while (attempt < max_attempts && running) {
        // Create a new socket for the reconnection attempt
        int new_socket = socket(AF_INET, SOCK_STREAM, 0);
        if (new_socket < 0) {
            NS_LOG_ERROR("Error creating socket for reconnection: " << strerror(errno));
            break;
        }
        
        // Set socket options for reconnection
        int opt = 1;
        ...
        if (connect(new_socket, (struct sockaddr*)&server_addr, sizeof(server_addr)) < 0) {
            NS_LOG_WARN("Reconnection attempt " << (attempt + 1) << " failed: " << strerror(errno));
            close(new_socket);
            
            // Wait before retrying
            if (attempt < max_attempts - 1) {
                NS_LOG_INFO("Retrying in " << retry_delay_seconds << " seconds...");
        ...
        NS_LOG_INFO("Successfully reconnected to RL agent on attempt " << (attempt + 1));
        
        // Send a reconnect notification to the agent
        Json::Value reconnect_msg;
        reconnect_msg["type"] = "reconnect";
        reconnect_msg["message"] = "Simulation reconnected successfully";
        
        Json::StreamWriterBuilder builder;
        std::string msg_str = Json::writeString(builder, reconnect_msg);
        
        if (send(client_socket, msg_str.c_str(), msg_str.length(), 0) < 0) {
            NS_LOG_WARN("Failed to send reconnect message: " << strerror(errno));
        }
    ...
    if (attempt >= max_attempts) {
        NS_LOG_ERROR("Failed to reconnect after " << max_attempts << " attempts. Giving up.");
    }
}
void RLInterface::SetAPPowerLevel(uint32_t ap_id, double power_level_dbm) {
    if (ap_id >= env->apNodes.GetN()) {
        NS_LOG_WARN("SetAPPowerLevel: Invalid AP ID " << ap_id);
        return;
    }

    Ptr<Node> apNode = env->apNodes.Get(ap_id);
    for (uint32_t i = 0; i < apNode->GetNDevices(); ++i) {
        Ptr<NetDevice> device = apNode->GetDevice(i);
        Ptr<WifiNetDevice> wifiDevice = DynamicCast<WifiNetDevice>(device);
        if (wifiDevice) {
            Ptr<WifiPhy> phy = wifiDevice->GetPhy();
            if (phy) {
                phy->SetTxPowerStart(power_level_dbm);
                phy->SetTxPowerEnd(power_level_dbm);
                NS_LOG_INFO("Set AP " << ap_id << " power to " << power_level_dbm << " dBm");
                return;
            }
        }
    }
    NS_LOG_WARN("Could not find WifiPhy for AP " << ap_id);
}
void RLInterface::ApplyChannelAssignment(const Json::Value& channels) {
    std::lock_guard<std::mutex> lock(m_dataMutex); 
    for (uint32_t i = 0; i < channels.size() && i < env->nAPs; ++i) {
        uint32_t new_channel = channels[i].asUInt();
        env->apChannelAssignment[i] = new_channel;
        NS_LOG_DEBUG("AP " << i << " assigned to channel " << new_channel);
    }
}
void RLInterface::ApplyPowerLevels(const Json::Value& power_levels) {
    for (uint32_t i = 0; i < power_levels.size() && i < env->nAPs; ++i) {
        double new_power = power_levels[i].asDouble();
        SetAPPowerLevel(i, new_power);
        NS_LOG_DEBUG("AP " << i << " power set to " << new_power);
    }
}
// Improve bounds checking in various methods
double RLInterface::GetAPUtilization(uint32_t ap_id) {
    if (ap_id >= env->nAPs) {
        NS_LOG_WARN("GetAPUtilization: Invalid AP ID " << ap_id);
        return 0.0;
    }
    
    auto it = env->channelUtilization.find(env->apChannelAssignment[ap_id]);
    return (it != env->channelUtilization.end()) ? it->second : 0.0;
}

uint32_t RLInterface::GetNumUsers(uint32_t ap_id) {
    if (ap_id >= env->nAPs) {
        NS_LOG_WARN("GetNumUsers: Invalid AP ID " << ap_id);
        return 0;
    }
    
    uint32_t count = 0;
    uint32_t start_index = ap_id * env->maxUsersPerAP;
    uint32_t end_index = start_index + env->maxUsersPerAP;
    
    // Ensure we don't exceed staActiveStatus bounds
    end_index = std::min(end_index, static_cast<uint32_t>(env->staActiveStatus.size()));
    
    for (uint32_t j = start_index; j < end_index; ++j) {
        if (env->staActiveStatus[j]) {
            count++;
        }
    }
    return count;
}
double RLInterface::GetAverageDelay(uint32_t ap_id) {
    auto it = env->apQueueDelay.find(ap_id);
    return (it != env->apQueueDelay.end()) ? it->second : 0.0;
}
double RLInterface::GetPowerConsumption(uint32_t ap_id) {
    if (ap_id < env->energySources.GetN()) {
        Ptr<BasicEnergySource> source = DynamicCast<BasicEnergySource>(env->energySources.Get(ap_id));
        if (source) {
            return source->GetInitialEnergy() - source->GetRemainingEnergy();
        }
    }
    return 0.0;
}
double RLInterface::GetInterferenceLevel(uint32_t ap_id) {
    uint32_t channel = env->apChannelAssignment[ap_id];
    auto it = env->channelInterference.find(channel);
    return (it != env->channelInterference.end()) ? it->second : 1.0;
}
double RLInterface::GetThroughputFromFlowMonitor() {
    if (!env->monitor){ 
        NS_LOG_WARN("Getthroughputfromflowmonitor: FlowMonitor is not initialized");
        return 0.0;
    }
    double total_throughput = 0.0;
    auto stats = env->monitor->GetFlowStats();
    for (auto const& [key, val] : stats) {
        double duration = (val.timeLastRxPacket - val.timeFirstTxPacket).GetSeconds();
        if (duration > 0.1) {
            total_throughput += (val.rxBytes * 8.0) / (duration * 1e6); // Mbps
        }
    }
    return total_throughput;
}
double RLInterface::CalculateTotalThroughput() {
    return env->monitor ? GetThroughputFromFlowMonitor() : 0.0;
}
double RLInterface::CalculatePacketLossRate() {
    if (!env->monitor) {
        NS_LOG_WARN("CalculatePacketLossRate: FlowMonitor is not initialized.");
        return 0.0;
    }
    uint64_t total_tx = 0;
    uint64_t total_rx = 0;
    auto stats = env->monitor->GetFlowStats();
    for (auto const& [key, val] : stats) {
        total_tx += val.txPackets;
        total_rx += val.rxPackets;
    }
    return (total_tx > 0) ? static_cast<double>(total_tx - total_rx) / total_tx : 0.0;
}
double RLInterface::CalculateTotalEnergyConsumption() {
    double total = 0.0;
    for (uint32_t i = 0; i < env->energySources.GetN(); ++i) {
        total += GetPowerConsumption(i);
    }
    return total;
}
double RLInterface::CalculateEnergyEfficiency() {
    double throughput = CalculateTotalThroughput();
    double energy = CalculateTotalEnergyConsumption();
    return (energy > 0) ? throughput / energy : 0.0;
}
double RLInterface::CalculateAverageQoS() {
    return 0.8; // Placeholder
}
double RLInterface::CalculateInterference(uint32_t ap1, uint32_t ap2) {
    if (ap1 == ap2) return 0.0;
    Vector pos1 = env->apNodes.Get(ap1)->GetObject<MobilityModel>()->GetPosition();
    Vector pos2 = env->apNodes.Get(ap2)->GetObject<MobilityModel>()->GetPosition();
    double distance = sqrt(pow(pos1.x - pos2.x, 2) + pow(pos1.y - pos2.y, 2));
    if (env->apChannelAssignment[ap1] == env->apChannelAssignment[ap2]) {
        return 1.0 / (1.0 + distance / 100.0);
    }
    return 0.1 / (1.0 + distance / 100.0);
}

// --- OptimizedWiFiEnvironment Method Implementations ---
...
OptimizedWiFiEnvironment::OptimizedWiFiEnvironment() {
    ...
    staActiveStatus.resize(nAPs * maxUsersPerAP, false);
...
void OptimizedWiFiEnvironment::ResetEpisode() {
    m_episodeStartTime = Simulator::Now();
    m_currentEpisodeStartHour = std::rand() % 24;
    NS_LOG_INFO("--- New Episode Start ---");
    NS_LOG_INFO("Resetting episode state. Real-world start time: " << m_currentEpisodeStartHour << ":00");

    for (uint32_t i = 0; i < clientApps.GetN(); ++i) {
        clientApps.Get(i)->SetStopTime(Simulator::Now());
    }
    clientApps = ApplicationContainer(); // Correct way to clear the container

    std::fill(staActiveStatus.begin(), staActiveStatus.end(), false);
    ScheduleTimeVaryingTraffic();
}

double OptimizedWiFiEnvironment::GetCurrentHourMultiplier() const {
    double elapsedSecondsInEpisode = (Simulator::Now() - m_episodeStartTime).GetSeconds();
    double realWorldSecondsElapsed = elapsedSecondsInEpisode * timeScalingFactor;
    uint32_t currentHour = (static_cast<uint32_t>(realWorldSecondsElapsed / 3600.0) + m_currentEpisodeStartHour) % 24;
...
void OptimizedWiFiEnvironment::SetupTopology() {
    ...
    std::mt19937 gen(42);
    std::discrete_distribution<> deviceDist({30, 40, 20, 10}); 
    
    for (uint32_t i = 0; i < staNodes.GetN(); ++i) {
        staDeviceTypes[i] = static_cast<DeviceType>(deviceDist(gen));
    }
}
void OptimizedWiFiEnvironment::ValidateConfiguration() {
    if (nAPs == 0) {
        NS_LOG_ERROR("Number of APs cannot be zero");
        exit(EXIT_FAILURE);
    }
    
    if (maxUsersPerAP == 0) {
        NS_LOG_ERROR("Max users per AP cannot be zero");
        exit(EXIT_FAILURE);
    }
    
    if (availableChannels.empty()) {
        NS_LOG_ERROR("Available channels list cannot be empty");
        exit(EXIT_FAILURE);
    }
    
    if (campusWidth <= 0 || campusHeight <= 0) {
        NS_LOG_ERROR("Campus dimensions must be positive");
        exit(EXIT_FAILURE);
    }
    
    if (simTime <= 0) {
        NS_LOG_ERROR("Simulation time must be positive");
        exit(EXIT_FAILURE);
    }
...
void OptimizedWiFiEnvironment::SetupHeterogeneousWiFi() {
    ...
    channelHelper.AddPropagationLoss("ns3::LogDistancePropagationLossModel",
                                     "Exponent", DoubleValue(3.0), // Reduced from 3.0
...
DeviceType OptimizedWiFiEnvironment::AssignDeviceType(uint32_t staIndex) {
    ...
    NS_LOG_WARN("staIndex " << staIndex << " out of bounds, defaulting to smartphone");
...
void OptimizedWiFiEnvironment::SetupAnimation(AnimationInterface& anim) {
    ...
    for (uint32_t i = 0; i < apNodes.GetN(); ++i) {
        ...
        uint8_t channelColors[7][3] = {
            {0, 0, 255}, {0, 100, 255}, {0, 150, 255}, {0, 200, 255},
            {50, 0, 255}, {100, 0, 255}, {150, 0, 255}
        };
...
int main(int argc, char *argv[]) {
    ...
    // Disable verbose application logging for better performance
    ...
    uint32_t scaledWifiQueue = std::min(50000u, std::max(1000u, static_cast<uint32_t>(100 * timeScalingFactor)));
    uint32_t scaledBackboneQueue = std::min(100000u, std::max(5000u, static_cast<uint32_t>(1000 * timeScalingFactor)));
    ...
    // --- Command Line Parsing for RL ---
    bool enable_rl = false;
    int rl_port = 9999;
    CommandLine cmd(__FILE__);
    cmd.AddValue("enable_rl", "Enable Reinforcement Learning interface", enable_rl);
    cmd.AddValue("port", "Port for RL communication", rl_port);
    cmd.Parse(argc, argv);
    ...
    if (enable_rl) {
        std::cout << "• Reinforcement Learning Interface ENABLED on port " << rl_port << std::endl;
    } else {
        std::cout << "• Reinforcement Learning Interface DISABLED" << std::endl;
    }
    std::cout << "===================================================\n" << std::endl;
    ...
    // --- RL Interface Setup ---
    RLInterface rl_interface(rl_port, &env);
    if (enable_rl) {
        rl_interface.EnableRLInterface(true);
        rl_interface.StartRLInterface();
    }
    
    // --- Animation Setup ---
    bool enAnim = false; // Disable animation for RL training runs
    if(enAnim){
